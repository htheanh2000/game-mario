...
void CMario::Update(DWORD dt, vector<LPGAMEOBJECT> *coObjects)
{
	...
	if(GetTickCount64() - untouchable_start > 2000) {
		untouchable_start = -1;
		untouchable = 0;
	}
...
void CMario::OnCollisionWithGoldBrick(LPCOLLISIONEVENT e)
{
	...
	if (goldBrick->objType == GOLD_BRICK_P_BUTTON && e->ny < 0)
	{ // Mario nhảy lên
		goldBrick->SetPress();
	}
}
...
void CMario::Render()
{
	if (untouchable)
	{
		ULONGLONG tickInterval = 200;
		ULONGLONG currentTick = GetTickCount64();
		// Check if enough time has elapsed
		if (GetTickCount64() - lastTick >= tickInterval)
		{
